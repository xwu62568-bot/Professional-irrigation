import type { Device, Field, Plan, Strategy } from './models';
...
export function buildFieldRisks(fields: Field[]): FieldRisk[] {
  return [...fields]
    .map((field) => {
      ...
      const riskLevel: FieldRisk['riskLevel'] =
        riskScore >= 55 ? '高' : riskScore >= 30 ? '中' : '低';
      const riskReason =
...
export function buildWeatherOverview(fields: Field[], rainToday = 0): WeatherOverview {
  const avgEtc = avg(fields.map((field) => field.etc));
  const next24hRainMm = Number((rainToday + avgEtc * 0.35).toFixed(1));
  const rainProbability = next24hRainMm > 1 ? 45 : 18;
  const recommendation =
    next24hRainMm >= avgEtc ? '可观望' : next24hRainMm > 1 ? '注意雨量折减' : '需按计划补水';

  return {
    todayRainMm: rainToday,
    next24hRainMm,
    rainProbability,
    recommendation,
  };
