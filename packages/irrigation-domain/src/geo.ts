...
export function applyVisualGeometryFromGeo<T extends Pick<Field, 'id' | 'polygon' | 'center' | 'zones'>>(
  ...
  function toVisualPoint(point: GeoPoint) {
  ...
  return fields.map((field) => {
    ...
    const polygon = shape.boundary.map((point) => toVisualPoint(point));
    const center = toVisualPoint(computeGeoCenter(shape.boundary) ?? shape.boundary[0]);
    ...
    return {
      ...
      zones: field.zones.map((zone) => {
        ...
        return {
          ...zone,
          polygon: zoneBoundary.map((point) => toVisualPoint(point)),
          center: toVisualPoint(computeGeoCenter(zoneBoundary) ?? zoneBoundary[0]),
        };
      }),
    };
  });
